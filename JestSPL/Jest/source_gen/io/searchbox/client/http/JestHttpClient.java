...
public class JestHttpClient extends AbstractJestClient {
  ...
  protected <T extends JestResult> Future<HttpResponse> executeAsyncRequest(Action<T> clientRequest, JestResultHandler<? super T> resultHandler, HttpUriRequest request) {
    if (httpClientContextTemplate != null) {
      return asyncClient.execute(request, createContextInstance(), new JestHttpClient.DefaultCallback<T>(clientRequest, request, resultHandler));
    }
    return asyncClient.execute(request, new JestHttpClient.DefaultCallback<T>(clientRequest, request, resultHandler));
  }
  ...
  private <T extends JestResult> T deserializeResponse(HttpResponse response, final HttpRequest httpRequest, Action<T> clientRequest) throws IOException {
    ...
    try {
      return clientRequest.createNewElasticSearchResult(((response.getEntity() == null ? null : EntityUtils.toString(response.getEntity()))), statusLine.getStatusCode(), statusLine.getReasonPhrase(), gson);
    } catch (JsonSyntaxException e) {
      for (Header header : response.getHeaders("Content-Type")) {
        final String mimeType = header.getValue();
        if (!(mimeType.startsWith("application/json"))) {
          final String message = "Request " + httpRequest.toString() + " yielded " + mimeType + ", should be json: " + statusLine.toString();
  ...
  protected class DefaultCallback<T extends JestResult> implements FutureCallback<HttpResponse> {
    ...
    public void failed(final Exception ex) {
      JestHttpClient.log.error("Exception occurred during async execution.", ex);
