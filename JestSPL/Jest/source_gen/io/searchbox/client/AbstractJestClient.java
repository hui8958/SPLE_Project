...
public abstract class AbstractJestClient implements JestClient {
  ...
  private final AtomicReference<AbstractJestClient.ServerPool> serverPoolReference = new AtomicReference<AbstractJestClient.ServerPool>(new AbstractJestClient.ServerPool((Set) ImmutableSet.of()));
  ...
  public void setServers(Set<String> servers) {
    if (servers.equals(serverPoolReference.get().getServers())) {
      if (log.isDebugEnabled()) {
        log.debug("Server pool already contains same list of servers: {}", StringUtils.join(servers, ","));
      }
      return;
    }
    if (log.isInfoEnabled()) {
      log.info("Setting server pool to a list of {} servers: [{}]", servers.size(), StringUtils.join(servers, ","));
    }
    serverPoolReference.set(new AbstractJestClient.ServerPool(servers));
    if (servers.isEmpty()) {
